...
query_wb = st.text_input(
    "🔍 Cari ribuan indikator World Bank (misal: 'gdp', 'inflation', 'poverty', 'education', 'co2'):",
    value="gdp growth"
).strip()

if query_wb and all_wb_indicators:
    # Pencarian fleksibel: mencocokkan kata kunci di dalam nama indikator
    ...
    if results_wb:
        ...
        # Tampilan dropdown bersih hanya menampilkan nama indikator
        options_wb = {ind['name']: ind for ind in results_wb}
        selected_wb_label = st.selectbox("Pilih Indikator dari Hasil Pencarian:", list(options_wb.keys()))
            ...
            with st.spinner(f"Menarik time-series untuk {selected_wb_label}..."):
                ...
                try:
                    ...
                    if records_wb:
                        ...
                        st.warning("Indikator ini terdaftar, tetapi observasi data runtun waktu untuk Indonesia tidak tersedia pada seri ini.")
